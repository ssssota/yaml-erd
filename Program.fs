...
let private stringOfFormat = function
  | PNG -> "png"
  | SVG -> "svg"
  | PDF -> "pdf"
...
let private compile args =
  let command = "dot"
  let pArgs = String.Format("-T{0} {1} -o {2} {3}", stringOfFormat args.Format, args.Temp, args.Output, args.AdditionalDotParams)
  let procStartInfo = ProcessStartInfo(FileName = command, Arguments = pArgs, WorkingDirectory = ".", UseShellExecute = false, RedirectStandardOutput = true, RedirectStandardError = true)
  if args.Verbose then
    printfn "[RUN] %s %s" command pArgs
  try
...
let main args =
  ...
  try
    let initArgs = { Input = "";  Temp = ""; Output = "./output.png"; Format = PNG; AdditionalDotParams = ""; Verbose = false}
    let args = List.fold (fun args -> function
      | Input path -> args |> Result.map (fun args -> { args with Input = path })
      | Temp path -> args |> Result.map (fun args -> { args with Temp = path })
      | Output path -> args |> Result.map (fun args -> { args with Output = path })
      | Format format -> args |> Result.bind (fun args -> formatOfString format |> Result.map (fun format -> { args with Format = format }))
      | Additional_Dot_Args dotArg -> args |> Result.map (fun args -> { args with AdditionalDotParams = args.AdditionalDotParams + " " + dotArg })
      | Verbose -> args |> Result.map (fun args -> { args with Verbose = true })) <| Ok initArgs <| (parser.Parse args).GetAllResults()
    let args =
      match args with
      | Ok args -> args
      | Error err ->
    ...
    let temp = if args.Temp = "" then System.IO.Path.GetTempFileName() else args.Temp
    let args = { args with Temp = temp }

    match Parse.schemaFromFile args.Input |> Util.Result.bind Validation.validate with
