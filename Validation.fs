...
type UnknownEntityError = { Name: string; Pos: Position }
...
type ValidationError =
    ...
    override self.ToString() =
        ...
        | UnknownEntity err ->
            String.Format(
                "\x1b[31m validation error[{0}:{1}-{2}:{3}]: \x1b[0m there is no entity named `{4}`",
                err.Pos.StartLine,
                err.Pos.StartColumn,
                err.Pos.EndLine,
                err.Pos.EndColumn,
        ...
        | UnknownField err ->
            String.Format(
                "\x1b[31m validation error[{0}:{1}-{2}:{3}]: \x1b[0m entity `{4}` does not have field `{5}`",
                err.Pos.StartLine,
                err.Pos.StartColumn,
                err.Pos.EndLine,
                err.Pos.EndColumn,
        ...
        | MutualRelations err ->
            String.Format(
                ...
                err.RefPos1.StartLine,
                err.RefPos1.StartColumn,
                err.RefPos1.EndLine,
                err.RefPos1.EndColumn,
                err.RefPos1.StartLine,
                err.RefPos1.StartColumn,
                err.RefPos1.EndLine,
                err.RefPos1.EndColumn,
...
let rec private hasStructField (strct: Struct) (field: string list): bool =
    ...
    | Record (fields_, _) ->
        not (List.isEmpty field)
        && List.exists (fun (key, v) -> key = field.Head && hasStructField v field.Tail) fields_
...
let validateRelation (schema: Schema.T): obj list * obj list =
    List.fold
        (fun acc (entity: Entity) ->
            List.fold
                (fun (accWarnings, accErrors) (relation: Relation) ->
                    let accErrors =
                        List.fold
                            (fun acc src ->
                    ...
                    let accErrors =
                        List.fold
                            (fun acc distField ->
                                ...
                                if hasEntityField schema distEntity distField then
                                    acc
                                else
                                    UnknownField
                                        { EntityName = distEntity
                                          FieldName = distField
                                          Pos = relation.Pos }
                                    :> obj
                                    :: acc)
                        ...
                        | Some distEntity ->
                            List.fold
                                (fun (acc: obj option) distRelation ->
                                    ...
                                    | Some x -> Some x
                                    | None ->
                                        if fst distRelation.Dist <> entity.Name then
                                            None
                                        else
                                            let forward =
                                                List.tryFind
                                                    (fun dist -> List.contains dist distRelation.Src)
                                                    (snd relation.Dist)

                                            let backward =
                                                List.tryFind
                                                    (fun dist -> List.contains dist relation.Src)
                                                    (snd distRelation.Dist)
