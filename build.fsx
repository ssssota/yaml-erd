...
Target.create "Test" (fun _ ->
  ...
  |> Seq.iter (DotNet.build id)
  Trace.log "-=-=-=-=-=-=-= Run Tests =-=-=-=-=-=-=-"
  testProjects |> Seq.iter (fun proj ->
...
Target.create "Publish" (fun _ ->
  ...
  |> Seq.iter (fun info ->
    ...
    |> Seq.iter (fun runtime ->
      ...
      let srcPath = sprintf "%s/%s" outputPath <| if runtime.Contains "win" then info.ProjectName + ".exe" else info.ProjectName
      let distPath = sprintf "%s/%s" outputPath <| if runtime.Contains "win" then info.OutputBinary + ".exe" else info.OutputBinary
